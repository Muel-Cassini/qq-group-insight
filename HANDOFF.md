...
>
> ⚠️ **这是开发/交接文档，面向维护者**。文中提到的 `data/`、`artifacts/`、`_recon/` 等目录
> **不在本仓库里**（见 `.gitignore`），它们指本地开发环境；真实数据与开发脚本一律不发布。
> 文中的路径统一写成 `D:\summary-workspace`，实际以你本机的克隆位置为准。
...
| 项 | 值 |
|---|---|
| 项目根 | `D:\summary-workspace`（发布副本是 git 仓库；原始工作区没有 `.git`） |
...
- 项目根 = 最近的含 `.git` 的祖先目录，否则是 cwd（本项目根锚定在克隆目录，所以启动控制台时 cwd 必须是项目根，否则技能加载不到）。
