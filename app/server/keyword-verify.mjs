...
import { ensureCrawlEdge, edgeInstance, listCdpTabs, closeCrawlTabs, findEdgePath } from './startup.mjs';
import {
...
/**
 * 起一个「登录状态检查」子任务。
 * ★ 检查的是**采集实例**（crawlPort/crawlProfile），并通过环境变量把端口/浏览器
 *   传给 check_login.py（它内部用 MediaCrawler 的 CDPBrowserManager，端口可被
 *   sitecustomize.py 读取的 DSH_CDP_PORT 覆盖）。
 */
export async function startLoginCheckJob({ platforms, ensureInstance = true, config = null } = {}) {
  const script = projectPath('app/server/tools/check_login.py');
  if (!fs.existsSync(script)) throw new Error('缺少 app/server/tools/check_login.py');
  ...
  return runProcess({
    ...
    command: pythonPath(),
    args: [script, ...platforms],
    cwd: projectPath(PATHS.mediaCrawler),
    env: {
      DSH_CDP_PORT: String(inst.port),
      DSH_CDP_CONNECT_EXISTING: '1',
      ...(edgePath ? { DSH_CDP_BROWSER_PATH: edgePath } : {}),
    },
    meta: { platforms, port: inst.port, instance: 'crawl', refresh: ['login'] },
    detect: [/\{"ok".*\}$/],
  });
}

/** 从任务日志里解析 check_login.py 的 JSON 结果 */
export function parseLoginResult(job) {
  const text = (job.logs || []).map((entry) => entry.line).join('\n');
  const line = text.split('\n').reverse().find((item) => item.trim().startsWith('{') && item.includes('platforms'));
  if (!line) return { ok: false, error: '登录检查未返回结果', platforms: {} };
  try {
...
export async function checkLogins(platforms, { onLine } = {}) {
  let job;
  try {
    job = await startLoginCheckJob({ platforms });
  } catch (err) {
    onLine?.(`[登录检查] ${err.message}`, 'err');
    return { ok: false, error: err.message, platforms: {} };
  }
  const done = await waitJobWithTimeout(job, { timeoutMs: 90 * 1000, label: '登录检查' });
  if (done.status !== 'done') onLine?.(`[登录检查] 未正常结束（${done.error || done.status}）`, 'err');
  return parseLoginResult(job);
}
...
export async function startKeywordVerify(opts) {
  ...
  return runTask({
    ...
    // 前置：确保**采集实例**在跑（与界面实例隔离，不占用你看界面的那个窗口）
    job.emit('[前置] 检查采集用的 Edge 实例…', 'sys');
    const edge = await ensureCrawlEdge({});
    if (!edge.ok) {
      job.emit(`[错误] 采集用的 Edge 实例不可用：${edge.reason || '未知原因'}。`
        + '可到「⑥ 设置 → 平台登录」点「启动采集实例」，或检查 Edge 是否安装。', 'err');
      throw new Error(`采集用的 Edge 实例不可用：${edge.reason || '未知原因'}`);
    }
    job.emit(`[前置] 采集实例就绪（端口 ${edge.port}｜profile ${edge.profile}｜${edge.version || '版本未知'}）`, 'sys');

    // 记下采集开始前采集实例里已有的标签页：收尾时只关「本次新出现的」，绝不动你自己开的页面
    let tabsBeforeRun = [];
    try {
      tabsBeforeRun = (await listCdpTabs(edge.port)).map((tab) => tab.id);
    } catch { /* 取不到就退化为不做整体收尾，每个采集任务结束仍会各自清理 */ }
    ...
    // 收尾清理：关掉采集实例里「本次运行新打开、可能残留」的社媒标签页
    // （每个采集任务结束时已经各自清过一轮，这里兜底；beforeIds 保证不会碰你自己开的页面）
    try {
      const inst = await edgeInstance('crawl');
      const swept = await closeCrawlTabs({
        port: inst.port,
        beforeIds: tabsBeforeRun,
        platforms: usable,
        onLine: (line, level) => job.emit(line, level || 'sys'),
      });
      if (swept?.closed) job.emit(`[清理] 收尾关闭 ${swept.closed} 个残留社媒标签页`, 'sys');
    } catch (err) {
      job.emit(`[提示] 收尾清理标签页失败（不影响结果）：${err.message}`, 'sys');
    }
