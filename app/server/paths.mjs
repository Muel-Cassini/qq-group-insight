...
export const DEFAULT_CONFIG = {
  ...
  startup: {
    ...
    dshPort: 3080,
    cdpPort: 9222,         // ★ 界面实例：控制台页面开在这个实例里（沿用历史配置）
    edgeProfile: '',       // 空 = %LOCALAPPDATA%\dsh-edge-cdp
    crawlPort: 9333,       // ★ 采集实例：社媒采集与登录判定专用，界面窗口不被它占用
    crawlProfile: '',      // 空 = %LOCALAPPDATA%\dsh-edge-crawl
    closeTabsAfterCrawl: true, // 采集结束后自动关掉它打开的社媒标签页
  },
  ai: {
    activeProviderId: '',
    ...
    maxTokens: 4096,       // 单次回答的输出上限；推理型模型容易被它截断（表现为"正文为空"），可调到 8192
  },
  classify: {
...
/** 界面实例的 profile（控制台页面开在这里） */
export function defaultEdgeProfile() {
  const base = process.env.LOCALAPPDATA || path.join(process.env.USERPROFILE || '', 'AppData', 'Local');
  return path.join(base, 'dsh-edge-cdp');
}

/** 采集实例的 profile（社媒登录态存在这里，与界面实例隔离、可随时删掉重来） */
export function defaultCrawlProfile() {
  const base = process.env.LOCALAPPDATA || path.join(process.env.USERPROFILE || '', 'AppData', 'Local');
  return path.join(base, 'dsh-edge-crawl');
}
