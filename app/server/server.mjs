...
import {
  ensureEdge, ensureCrawlEdge, ensureDsh, bootstrap, statusSnapshot, openPath, portOpen,
} from './startup.mjs';
...
import { crawlBrowserStatus, startLoginStatusCheck, openLoginPage } from './login.mjs';
...
const routes = {
  ...
  'POST /api/provider': async (body) => {
    ...
    // API Key 的语义（此前有坑：改模型时前端把空串发上来 → Key 被误删 → 界面显示"未配置"）：
    //   非空 apiKey     → 写入新 Key
    //   apiKey 为空/不传 → **保持原样**（弹窗标签写的就是「留空表示不修改」）
    //   clearApiKey=true → 显式清空（界面上有单独的勾选项）
    ...
    return { ok: true, data: { providers: next.ai.providers, secretState: hasSecret(`provider:${id}`), keyChanged } };
  },
  ...
  'POST /api/provider/models': async (body) => {
    ...
    if (!models.length) throw new Error('该服务没有返回任何模型（检查 Base URL / Key / 权限）');
    // 「拉取模型」要把结果**落到配置里**，否则用户拉完在界面上什么都看不到（此前只弹了个 toast）
    const config = await loadConfig();
    const providers = Array.isArray(config.ai.providers) ? [...config.ai.providers] : [];
    const index = providers.findIndex((item) => item.id === body.id);
    if (index < 0) throw new Error('未找到该服务配置，请先保存服务再拉取模型');
    const current = providers[index];
    let defaultModel = current.defaultModel || '';
    let defaultChanged = false;
    // 默认模型不在新列表里（或本来就没设）→ 自动指向第一个，避免调用时报"模型不存在"
    if (!defaultModel || !models.includes(defaultModel)) {
      defaultModel = models[0];
      defaultChanged = true;
    }
    providers[index] = { ...current, models, defaultModel };
    const next = await saveConfig({ ai: { providers } });
    return {
      ok: true,
      data: {
        models, defaultModel, defaultChanged,
        previous: current.defaultModel || '',
        providers: next.ai.providers,
      },
    };
  },
  ...
  // —— 平台登录（全部在「采集实例」里进行，不占用界面窗口）
  'GET /api/login/status': async () => ({ ok: true, data: await crawlBrowserStatus() }),

  'POST /api/login/status': async (body) => {
    const job = await startLoginStatusCheck({ platforms: body.platforms });
    return { ok: true, data: { jobId: job.id } };
  },

  'POST /api/login/open': async (body) => ({ ok: true, data: await openLoginPage({ platform: body.platform }) }),

  'POST /api/crawl-browser/start': async () => ({ ok: true, data: await ensureCrawlEdge({}) }),
