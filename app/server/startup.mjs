...
import { PROJECT_ROOT, PATHS, defaultEdgeProfile, defaultCrawlProfile } from './paths.mjs';
import { loadConfig } from './config.mjs';
import { runProcess } from './jobs.mjs';
import { urlMatchesPlatform } from './platforms.mjs';
...
/** msedge.exe 的完整路径（找不到返回空串）——给需要强制用 Edge 的子进程用 */
...
/**
 * 解析某个 Edge 实例的端口与 profile。
 * - which='ui'    ：界面实例（控制台页面开在这里）
 * - which='crawl' ：采集实例（社媒采集与登录判定用，界面窗口不被它占用）
 */
export async function edgeInstance(which = 'ui', { config } = {}) {
  const cfg = config || (await loadConfig());
  if (which === 'crawl') {
    return {
      which: 'crawl',
      port: Number(cfg.startup.crawlPort || 9333),
      profile: cfg.startup.crawlProfile || defaultCrawlProfile(),
    };
  }
  return {
    which: 'ui',
    port: Number(cfg.startup.cdpPort || 9222),
    profile: cfg.startup.edgeProfile || defaultEdgeProfile(),
  };
}

/** 确保采集实例在跑（采集/登录判定前调用；与界面实例完全隔离） */
export async function ensureCrawlEdge({ config } = {}) {
  return ensureEdge({ config, which: 'crawl' });
}

/**
 * 确保某个 Edge 调试实例在跑；返回 {ok, running, started, port, version, profile}
 * @param {'ui'|'crawl'} which
 */
export async function ensureEdge({ config, which = 'ui' } = {}) {
  const cfg = config || (await loadConfig());
  const { port, profile } = await edgeInstance(which, { config: cfg });
  if (await portOpen(port)) {
    let version = null;
    try {
      version = (await getJson(`http://127.0.0.1:${port}/json/version`))?.Browser || null;
    } catch { /* 端口通但取不到版本也能用 */ }
    return { ok: true, running: true, started: false, port, version, profile, which };
  }

  const script = path.join(PROJECT_ROOT, PATHS.edgeLauncher);
  if (fs.existsSync(script)) {
    const job = runProcess({
      kind: which === 'crawl' ? 'launch-edge-crawl' : 'launch-edge',
      title: which === 'crawl' ? '启动 Edge 采集实例' : '启动 Edge 界面实例',
      command: 'powershell',
      args: ['-NoProfile', '-ExecutionPolicy', 'Bypass', '-File', script, '-Port', String(port),
        '-UserDataDir', profile],
    });
    if (which === 'crawl') started.crawlJob = job.id;
    else started.edgeJob = job.id;
  } else {
    // 没有脚本时退回直接启动 Edge（同样带调试端口与独立 profile）
    const edge = findEdge();
    if (!edge) return { ok: false, reason: '未找到 msedge.exe', which };
  ...
  for (let i = 0; i < 30; i += 1) {
    ...
    if (await portOpen(port)) {
      ...
      if (which === 'crawl') started.crawl = true;
      else started.edge = true;
      return {
        ok: true, running: true, started: true, port, version, profile, which,
        jobId: which === 'crawl' ? started.crawlJob : started.edgeJob,
      };
    }
  }
  return { ok: false, reason: '等待 Edge 调试端口超时（30 秒）', port, which };
}

// ---------------------------------------------------------------- CDP 标签页

/** 列出实例里的页面标签页 */
export async function listCdpTabs(port) {
  const list = await httpJson({ port, path: '/json/list' });
  return (Array.isArray(list) ? list : []).filter((t) => t.type === 'page' && t.id);
}

/** 在实例里新开一个标签页 */
export async function openCdpTab(port, url) {
  return httpJson({ port, path: `/json/new?${encodeURIComponent(url)}`, method: 'PUT' });
}

/** 关闭一个标签页（新老 Chromium 分别用 PUT/GET，两个都试） */
export async function closeCdpTab(port, targetId) {
  try {
    return await httpJson({ port, path: `/json/close/${targetId}`, method: 'PUT' });
  } catch {
    return httpJson({ port, path: `/json/close/${targetId}`, method: 'GET' });
  }
}

/**
 * 关掉「采集刚打开的那些」社媒标签页。
 * 只关：① 不在 beforeIds 里的（即采集开始后新出现的）、② URL 属于指定平台域名的标签页。
 * 这样既不会碰用户自己开着的页面，也不会碰界面窗口（界面实例是另一个端口）。
 */
export async function closeCrawlTabs({ port, beforeIds = [], platforms = [], onLine = null } = {}) {
  const cfg = await loadConfig();
  if (cfg.startup.closeTabsAfterCrawl === false) {
    onLine?.('[提示] 配置里关闭了「采集后自动关标签页」，跳过清理', 'sys');
    return { closed: 0, skipped: true };
  }
  const wanted = (Array.isArray(platforms) ? platforms : [platforms]).filter(Boolean);
  if (!wanted.length) return { closed: 0, skipped: true, reason: '未指定平台' };

  let tabs = [];
  try {
    tabs = await listCdpTabs(port);
  } catch (err) {
    onLine?.(`[提示] 读取采集实例标签页失败，跳过清理：${err.message}`, 'sys');
    return { closed: 0, error: err.message };
  }
  const before = new Set(beforeIds);
  const targets = tabs.filter((tab) => !before.has(tab.id)
    && wanted.some((code) => urlMatchesPlatform(tab.url, code)));
  for (const tab of targets) {
    // eslint-disable-next-line no-await-in-loop
    try {
      await closeCdpTab(port, tab.id);
      onLine?.(`[清理] 已关闭采集标签页：${(tab.url || '').slice(0, 90)}`, 'sys');
    } catch (err) {
      onLine?.(`[提示] 关闭标签页失败（不影响结果）：${err.message}`, 'sys');
    }
  }
  return { closed: targets.length, kept: tabs.length - targets.length };
}
...
export async function statusSnapshot() {
  const config = await loadConfig();
  const ui = await edgeInstance('ui', { config });
  const crawl = await edgeInstance('crawl', { config });
  const dshPort = Number(config.startup.dshPort || 3080);
  const [uiUp, crawlUp, dsh] = await Promise.all([
    portOpen(ui.port), portOpen(crawl.port), portOpen(dshPort),
  ]);
  let uiVersion = null;
  let crawlVersion = null;
  if (uiUp) {
    try { uiVersion = (await getJson(`http://127.0.0.1:${ui.port}/json/version`))?.Browser || null; } catch { /* ignore */ }
  }
  if (crawlUp) {
    try { crawlVersion = (await getJson(`http://127.0.0.1:${crawl.port}/json/version`))?.Browser || null; } catch { /* ignore */ }
  }
  return {
    // edge 保持为「界面实例」的别名，向后兼容既有前端
    edge: { running: uiUp, port: ui.port, version: uiVersion, startedByApp: started.edge },
    crawl: {
      running: crawlUp, port: crawl.port, version: crawlVersion,
      profile: crawl.profile, startedByApp: started.crawl,
    },
    dsh: { running: dsh, port: dshPort, url: `http://127.0.0.1:${dshPort}`, startedByApp: started.dsh },
    edgeProfile: ui.profile,
  };
}
