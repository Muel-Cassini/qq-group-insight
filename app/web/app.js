...
async function loadExports() {
  try {
    ...
    const hiddenCount = data.items.length - STATE.exports.length;
...
function providerForm(provider = null, apiKeyMask = '') {
  ...
  return `
    ...
    <div class="row" style="margin-top:8px">
      <div class="field grow"><label>模型名（逗号分隔）</label><input type="text" id="pfModels" value="${escapeHtml((provider?.models || []).join(', '))}"></div>
    ...
    <div class="row" style="margin-top:8px">
      ...
      <button class="btn" id="pfPickModel">按模型名填充默认</button>
    </div>
    <div class="actions">
...
function bindSettings() {
  ...
  $('#btnNewProvider').onclick = async () => {
    modal(providerForm(null), {
      onOpen: () => {
        ...
        $('#pfPickModel').onclick = () => {
          const first = ($('#pfModels').value.split(',')[0] || '').trim();
          $('#pfDefault').value = first;
        };
            ...
            const data = await api('/api/provider', {
              ...
              body: {
                provider: {
                  ...
                  models: $('#pfModels').value.split(',').map((s) => s.trim()).filter(Boolean),
                  defaultModel: $('#pfDefault').value.trim(),
                },
                apiKey: $('#pfKey').value,
              },
            });
            closeModal();
            toast(`已保存服务（共 ${data.providers.length} 个）`, 'ok');
  ...
  $('#providerRows').addEventListener('click', async (event) => {
    ...
    if (edit) {
      ...
      modal(providerForm(provider, STATE.secretState[edit] ? '已配置，留空不修改' : ''), {
        onOpen: () => {
          ...
          $('#pfPickModel').onclick = () => {
            $('#pfDefault').value = ($('#pfModels').value.split(',')[0] || '').trim();
          };
          $('#pfSave').onclick = async () => {
            try {
    ...
    if (models) {
      try {
        const data = await api('/api/provider/models', { method: 'POST', body: { id: models } });
        toast(`拉取到 ${data.models.length} 个模型：${data.models.slice(0, 6).join(', ')}`, 'ok');
